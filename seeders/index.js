...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Marlee.Schoen' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'P7mxQCdzGnwlNWs',
        'isDeleted':false,
        'username':'Marlee.Schoen',
        'email':'Alexandrine.Grant@yahoo.com',
        'isActive':true,
      ...
      await dbService.update(model.user, { 'username':'Marlee.Schoen' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Justina.Witting59' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'M0ILgTDbNwBbkow',
        'isDeleted':false,
        'username':'Justina.Witting59',
        'email':'Keagan84@gmail.com',
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.update(model.user, { 'username':'Justina.Witting59' }, userToBeInserted);
    }
